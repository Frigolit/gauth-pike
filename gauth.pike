...
int main(int argc, array argv) {
	...
	string s_iss  = Getopt.find_option(argv, "i", "issuer", UNDEFINED, "My service");
	...
	if (f_help) {
		...
		werror("-i|--issuer <name>     Specify issuer name (for --url, default: \"My service\").\n");
...
string gauth_totp_url(string accountname, string|void issuer, string key) {
	...
	// Prepare key and account name for insertion into URL
	string s_key = encode_key_base32(key);
	string s_acc = Protocols.HTTP.percent_encode(accountname);

	if (issuer) {
		return sprintf("otpauth://totp/%s:%s?secret=%s&issuer=%[0]s", Protocols.HTTP.percent_encode(issuer), s_acc, s_key);
	}
	else {
		return sprintf("otpauth://totp/%s?secret=%s", s_acc, s_key);
	}
}
